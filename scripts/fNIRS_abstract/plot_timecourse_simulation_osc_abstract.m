...
OPTIONS.output_folder = fullfile('/Users/edelaire1/Documents/Etude/04_PHD/Abstract/2026_fNIRS_china/figures','Simulation','oscilation', sprintf('ROI-%d_SNR-%0.2fdB',ROI_ID,SNR));
...
sFiles{3} = {ResultFiles(contains({ResultFiles.Comment},{'wMEM'})  & contains({ResultFiles.Comment},{'norm = adaptive'})& contains({ResultFiles.Comment},{'nbo = 4'})).FileName };
...
function  plot_timecourse(SubjectName, sFiles, sFiles_label, OPTIONS)
    ...
    for k = 1:length(sFiles)
        ...
        hold on;
        plot(Time, HbO , 'DisplayName',[ sFiles_label{k} ' - WL 830'], 'LineWidth', OPTIONS.LineWidth, 'Color', OPTIONS.color_red(k,:));
