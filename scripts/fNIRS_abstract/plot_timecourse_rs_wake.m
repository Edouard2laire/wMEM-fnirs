...
OPTIONS.fontsize    = 22;
OPTIONS.output_folder = fullfile('/Users/edelaire1/Documents/Etude/04_PHD/Abstract/2026_fNIRS_china/figures','TFR');
...
power_spectrumsMNE = compute_power_spectrum(sFilesTF{2});
power_spectrumswMEM = compute_power_spectrum(sFilesTF{3});


WData = [cat(2,power_spectrumsMNE.WData)' ; cat(2,power_spectrumswMEM.WData)' ];
WDataStd  = [cat(2,power_spectrumsMNE.WDataStd)' ; cat(2,power_spectrumswMEM.WDataStd)' ];

options_spectrum = struct();
options_spectrum.colormap = 'jet';
options_spectrum.clim = [0 0.25];
options_spectrum.wavelet.display.fontscale = 22;
options_spectrum.wavelet.display.TaegerK = 'yes';



options_spectrum.color_map =   [
             145,191,219; ...
             252,141,89; ...
             69,117,180 ;...
             215,48,39 ; ...
...
options_spectrum.title_tf = 'Power-Spectrum';
hFig = figure('Units','pixels','Position', getFigureSize(30, 20));
set(hFig, 'PaperPositionMode', 'auto'); hold on;
options_spectrum.hFig = hFig;

fig = process_ft_wavelet('displayPowerSpectrum',  WData, ...
                                                  WDataStd, ...
                                                  {'Rest - MNE','Task - MNE', 'Rest - wMEM','Task - wMEM'} , ...
                                                  power_spectrumsMNE(1).freq , ...
                                                  options_spectrum);
...
saveas(fig,fullfile(OPTIONS.output_folder, 'power_spectrum_combined.svg'));
%close(fig)
...
OPTIONS.fig = hFig;
plot_timecourse_cortex(SubjectName, {sFiles{3}}, {'Reconstructed Time course (wMEM)'}, OPTIONS)
xlim([50, 1100]);
...
yticks(ax, [-8, -5, -3, 1]); yticklabels(ax, {'0.004', '0.031','0.125', '2.000'});

xlim(ax,[50, 1100]);
clim(ax, [0, 0.2])
colorbar(ax,'eastoutside');

title(ax,'Time-frequency representation', 'FontSize', options_spectrum.wavelet.display.fontscale + 5)
ax.TitleHorizontalAlignment = 'left';
...
function plot_timecouse_channels(channel_file, sFile, OPTIONS)
    ...
    if OPTIONS.plot_montage
        ...
        t1 = title(ax, 'a. Montage', 'FontSize', 25);
        ax.TitleHorizontalAlignment = 'left';
...
function  plot_timecourse_cortex(SubjectName, sFiles, sFiles_label, OPTIONS)
    ...
    assert(~isempty(ROI), 'Unable to find the ROI')
    ...
    for k = 1:length(sFiles)
        ...
        ax1 = nexttile(); 
        hold on;
        plot(Time, HbO , 'DisplayName',[ sFiles_label{k} ' - HbO'], 'LineWidth', OPTIONS.LineWidth, 'Color',OPTIONS.color_red(2,:));
        plot(Time, HbR , 'DisplayName',[ sFiles_label{k} ' - HbO'], 'LineWidth', OPTIONS.LineWidth, 'Color',OPTIONS.color_blue(2,:));
