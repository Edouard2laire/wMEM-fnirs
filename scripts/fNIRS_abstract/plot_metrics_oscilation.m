...
output_folder = fullfile('/Users/edelaire1/Documents/Etude/04_PHD/Abstract/2026_fNIRS_china/figures','Simulation','oscilation');
...
clear g
data_all.label = data_all.snr;
g(1,1) = plot_boxplot(data_all, {'DLE'},  'method'); g(1,1).set_title('a. DLE (mm)'); g(1,1).no_legend()
g(1,2) = plot_boxplot(data_all, {'SD'},  'method'); g(1,2).set_title('b. SD (mm)'); g(1,2).no_legend()
g(1,3) = plot_boxplot(data_all, {'correlation'},  'method'); g(1,3).set_title('c. Correlation (%)');

g(1,1).geom_hline("yintercept",5,"style", 'b--');
g(1,2).geom_hline("yintercept",10,"style", 'b--');
g(1,3).geom_hline("yintercept", 0.8,"style", 'b--');

g(1,:).set_title('Localization of simulated oscillatory activity')
...
disp('Stat on SD')
disp('-------------')
compute_stats(data_all, 'SD');

%%
disp('Stat on correlation')
disp('-------------')
compute_stats(data_all, 'correlation');


%%
disp('Stat on DLE')
disp('-------------')
compute_stats(data_all, 'DLE');
...
disp('-------------')
compute_stats(data_all_select, 'correlation');
