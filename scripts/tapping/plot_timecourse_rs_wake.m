...
ax = nexttile([2, 1]); 
title(ax, 'a. Montage', 'FontSize', 25);
...
t2 = title(ax, 'b. Theoretical Recording', 'FontSize', 25);
ax.TitleHorizontalAlignment = 'left';
set(ax,    'fontsize', OPTIONS.fontsize, 'LineWidth',OPTIONS.LineWidth);
set(gca,   'Color',    [1,1,1]);
set(gcf,   'color',   'w');
...
yticks(ax, [-8, -5, -3, 1]); yticklabels(ax, {'0.004', '0.031','0.125', '2.000'});
title(ax,'c. Time-frequency representation', 'FontSize', options_spectrum.wavelet.display.fontscale + 5)
ax.TitleHorizontalAlignment = 'left';
colorbar(ax,'eastoutside');
